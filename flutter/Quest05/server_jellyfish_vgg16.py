...
async def get_label():
    try:
        result = await vgg16_prediction_model.prediction_model(vgg16_model)
        logger.info("Label Prediction was requested")
        return {"predicted_label": result["predicted_label"]}
    except Exception as e:
        logger.error("Prediction failed: %s", e)
        raise HTTPException(status_code=500, detail="Internal Server Error")

#`/score` 엔드포인트 (예측된 확률만 반환)
@app.get("/score")
async def get_score():
    try:
        result = await vgg16_prediction_model.prediction_model(vgg16_model)
        logger.info("Score Prediction was requested")
        return {"prediction_score": result["prediction_score"]}
    except Exception as e:
        logger.error("Prediction failed: %s", e)
        raise HTTPException(status_code=500, detail="Internal Server Error")        
...
if __name__ == "__main__":
    uvicorn.run("server_jellyfish_vgg16:app",
